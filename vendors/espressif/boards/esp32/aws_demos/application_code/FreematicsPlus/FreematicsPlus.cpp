...
bool Task::create(void (*task)(void*), const char* name, int priority, int stacksize)
{
    ...
    BaseType_t xReturned = xTaskCreate(task, name, stacksize, (void*)this, priority, ( TaskHandle_t * ) ( &xHandle ));
...
void Task::suspend()
{
    if (xHandle) vTaskSuspend((TaskHandle_t)xHandle);
}

void Task::resume()
{
    if (xHandle) vTaskResume((TaskHandle_t)xHandle);
}
...
void Mutex::lock()
{
  xSemaphoreTake((QueueHandle_t)xSemaphore, portMAX_DELAY);
}
...
bool FreematicsESP32::begin(bool useGNSS, bool useCellular, bool useCoProc)
{
    ...
    if (useCoProc) do {
        ...
        delete linkUART;
        linkUART = 0;
        Serial.print("No UART connection\r\n");
        ...
        delete linkSPI;
        linkSPI = 0;
        Serial.print("No SPI connection\r\n");
    } while(1);
